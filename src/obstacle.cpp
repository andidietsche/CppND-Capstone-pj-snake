...
bool Obstacle::ObstacleCell(int x, int y){
    ...
    if(obstacles.empty()) {
        std::cout << "vecotr is empty" << std::endl;
        return false;
        } //checks if no obstacles have been placed
    
    else{
        std::cout << "vecotr is not empty" << std::endl;
...
void Obstacle::PlaceObstacle(Snake snake, int score, SDL_Point food){
    ...
    if(InsertNewobstacle(score)){// TODO add score DONE
        SDL_Point point;
        int x,y;
        //place it random
        while(true){
        ...
        if(!snake.SnakeCell(x,y) && x != food.x && y != food.y)
            ...
            else{for(SDL_Point const &item : obstacles)
            if(x != item.x && y != item.y){ //check if not on obstacle
            point.x = x;
            point.y = y;}
            break;
